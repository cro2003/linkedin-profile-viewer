...
PREFIX = "stat:"
TRACKED = ("requests", "cache_hits", "negative_hits", "jobs_queued", "fetches",
           "retries", "failures", "session_refreshes", "checkpoints", "rate_limited")
...
async def snapshot() -> dict[str, int]:
    ...
    return {name: int(v or 0) for name, v in zip(TRACKED, values)}
