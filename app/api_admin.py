...
router = APIRouter(prefix="/v1/admin", tags=["admin"],
                   dependencies=[Depends(auth.require_superadmin)])
...
async def add_account(body: AccountIn, request: Request):
    ...
    if body.mode == "cookies":
        if not body.cookies:
            raise HTTPException(422, {"code": "cookies_required",
                                      "message": "cookies are required in cookies mode"})
        try:
            await accounts.create(body.id, body.cookies, proxy_url=body.proxy_url,
                                  email=body.email, note=body.note or "added via admin")
        except ValueError as e:
            raise HTTPException(422, {"code": "invalid_cookies", "message": str(e)})
        return {"account_id": body.id, "status": "created"}

    if not (body.email and body.password):
        raise HTTPException(422, {"code": "credentials_required",
    ...
    await request.app.state.arq.enqueue_job(
        "add_account_job", login_id, body.id, body.email, body.password,
        body.proxy_url, body.note)
    # the password is passed to the job and never stored
    return {"login_id": login_id, "account_id": body.id, "status": logins.RUNNING,
...
async def login_status(login_id: str):
    ...
    if not status:
        raise HTTPException(404, {"code": "login_not_found",
                                  "message": "unknown or expired login attempt"})
    return status
...
async def patch_account(account_id: str, body: AccountPatch):
    ...
    await accounts.update(account_id, disabled=body.disabled, proxy_url=body.proxy_url,
                          note=body.note)
...
async def patch_config(values: dict):
    try:
        ...
        raise HTTPException(422, {"code": "invalid_config", "message": str(e)})
...
async def patch_user(user_id: str, body: UserPatch,
                     caller: auth.Caller = Depends(auth.require_superadmin)):
    ...
    if caller.user and caller.user["_id"] == user_id:
        if updates.get("disabled") or updates.get("role") == auth.ROLE_USER:
            raise HTTPException(422, {"code": "self_lockout",
                                      "message": "cannot disable or demote your own account"})
